	...
	go func() {
		...
		for {
			...
			go func() {
				...
				for {
					// // enquanto conexao aberta
					if err != nil {
						fmt.Println(".", err)
						break
					}
					bufTam := make([]byte, 4)
					// // le tamanho da mensagem
					...
					// declara buffer do tamanho exato
					_, err = io.ReadFull(conn, bufMsg)
					// le do tamanho do buffer ou da erro
					if err != nil {
						fmt.Println("@", err)
						break
					}
					...
					module.Ind <- msg
					// // repassa mensagem para modulo superior
				}
			}()
		}
	}()
...
func (module *PP2PLink) Send(message PP2PLink_Req_Message) {
	...
	if conn, ok = module.Cache[message.To]; ok {
		...
		// se nao existe, abre e guarda na cache
	...
	// escreve 4 caracteres com tamanho
	_, err = fmt.Fprintf(conn, message.Message)
	// escreve a mensagem com o tamanho calculado
	if err != nil {
